...
ggplot(tib, aes(N,dist,group=Quantile))+geom_line(aes(color=Quantile))+geom_point(aes(shape=Quantile))+geom_hline(yintercept = 0)+scale_y_continuous("KS-statistic")+scale_colour_manual(values=cbPalette)
