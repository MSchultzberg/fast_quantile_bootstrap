...
<<echo=FALSE, cache=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.asp=0.62>>=
temp<-cbind(table(index_med[,1]))
tib<-tibble(index=as.numeric(rownames(temp)), frequency=temp[,1])

ggplot(tib, aes(index,frequency))+geom_bar(stat = "identity")+scale_x_continuous("Index of order statistic in the original sample",breaks=seq(140,260,10), labels = seq(140,260,10))
@
...
ggplot(tib, aes(x=index,group=Distribution))+geom_density(aes(color=Distribution), size=1)+scale_x_continuous("Index of order statistic in the original sample")+theme(legend.position = "bottom")
